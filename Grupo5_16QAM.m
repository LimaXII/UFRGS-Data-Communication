...
aux_ldpc.load_wifi_ldpc(N, R);       % Carregar o código LDPC correspondente ao H_1944_1_2.
H = sparse(logical(aux_ldpc.H));     % Exibir a matriz H gerada.
...
for i = 1:length(Eb_N0_lin)
    ...
    for j = 1:blocks
        ...
        rx_symbols_uncode = mod_symbols_uncode + noise;                         % Sinal recebido sem código.
...
grid on;
title('Desempenho do sistema 16QAM com LDPC (BER)');
...
legend('Location', 'southwest');
grid on;
title('Desempenho do sistema 16QAM com LDPC (FER)');
